---
title: "[GitAIOps 스터디 2주차] 배포 파이프라인과 관측 가능성 — Git Push 한 번으로 배포되고, 무슨 일이 일어나는지 다 보이게"
...
1주차에 Claude Code가 알아서 배포까지 해줬을 때는 사실 되게 만족스러웠다. 그런데 3.1의 "설정 드리프트"와 "왜의 실종" 이야기를 읽으면서 뜨끔했다. **편하게 배포되는 것과, 그 배포가 추적·재현·롤백 가능한 것은 전혀 다른 문제**였다. 회사에서 RPA나 자동화 스크립트를 돌릴 때도 "일단 돌아가면 됐지" 하고 넘어간 것들이 나중에 "이거 왜 이렇게 돼 있지?"로 돌아오는 경험이 많았는데, GitOps는 그 "왜"를 커밋 이력으로 강제로 남기게 만드는 구조라는 점이 인상 깊었다.
